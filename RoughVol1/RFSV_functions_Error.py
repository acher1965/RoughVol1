...
"""Error and cost calculation helpers for RFSV.

Provides functions to compute Monte Carlo implied-volatility errors and fitting cost.
"""
    ...
    for i in range(C_err.shape[0]):
        for j in range(C_err.shape[1]):
            if np.isnan(C_err[i, j]):
                ...
                IV_err[i, j] = C_err[i, j] / BSM_vega(
                    IV[i, j], S_0, K[i, j], expiries[i]
                )
    return IV_err
...
def fit_cost_calculation(IV, volgrid, weigthsgrid, tenor_len):
    ...
    for i in range(fit_cost_matr.shape[1]):
        ...
        fit_cost_exp[i] = np.sum(fit_cost_matr[:, i][tmp]) / np.sum(
            weigthsgrid[:, i][tmp]
        )
...
def cost_row_df_creation(cost_row, tenor_len):
    cost_exp_names = ["CostExpiry" + str(i) for i in range(1, tenor_len + 1)] + [
        "GlobalCost"
    ]
