...
def hard_coded_params() -> HardCodedParameters:
    """Hard-coded parameters"""
    N_treshold = 10
    dt_short, dt_inf, alpha = 1 / 25 / 365, 6 / 365, 0.5  # parameters for time steps
    strikes = np.arange(-2.5, 2.5 + 0.5, 0.5)  # in stdev terms
    near_atm_strikes = np.linspace(-0.5, 0.5, 11)[
        np.isin(np.linspace(-0.5, 0.5, 11), strikes, invert=True)
    ]
...
def calculate_request(
    hc: HardCodedParameters, row_inputs: Any, volgrid, weigthsgrid
) -> RFSV_BF_Results:
    ...
    [expiries, forward_input, xi_input] = arrays_control(
        r.expiries_nan, r.forward_input_nan, r.xi_input_nan
    )
    [expiries_nan, forward_input_nan, xi_input_nan] = arrays_for_output(
        expiries, forward_input, xi_input, hc.tenor_len
    )
    ...
    K, C, K_skew, C_skew, warn = MC_strike_and_price(
        S, r.S_0, expiries, t, hc.strikes, hc.dK_skew, xi, r.n, hc.N_treshold
    )
    IV, IV_skew = imp_vol_calculation(K, C, K_skew, C_skew, r.S_0, expiries)
    skew, smile = skew_smile_calculation(IV_skew, hc.dK_skew)
    sign_bound, Sigma0_d, Sigma0_dd, a_0 = Sigma_Taylor_coefficients(
        r.H, r.rho, r.eta, xi, expiries
    )
    IV_approx, skew_approx, smile_approx, IV_skew_approx = imp_vol_approx(
        K, K_skew, r.S_0, expiries, xi, r.H, r.rho, r.eta, hc.tenor_len
    )
    fit_cost_arr = fit_cost_calculation(IV, volgrid, weigthsgrid, hc.tenor_len)
    cost_row = np.vstack(
        [r.request_id, r.underlying, r.as_of, r.H, r.eta, r.rho, fit_cost_arr]
    )
    ...
    if r.full_diagn_flag == True:
        df_int_var_std = df_integ_var_std_creation(
            logS, logv, xi_input, xi_values, t, expiries, xi, r.H, r.eta, T
        )
    else:
    ...
    df_skew_smile = df_skew_smile_creation(
        skew, smile, IV_skew, IV_df, hc.strikes, hc.dK_skew, expiries_nan, hc.tenor_len
    )
    IV_approx_df, df_IV_approx = df_IV_creation(
        IV_approx, hc.strikes, hc.tenor_len, "Approx"
    )
    df_skew_smile_approx = df_skew_smile_creation(
        skew_approx,
        smile_approx,
        IV_skew_approx,
        IV_approx_df,
...
def parameter_with_H(H: float) -> Tuple[float, float]:
    gamma = 0.5 - H
    D_H = float(np.sqrt(2 * H) / (H + 0.5))
    return gamma, D_H


def t_from_dt_e(
    dt_short: float, dt_inf: float, alpha: float, e: NDArray[Any]
) -> Tuple[NDArray[np.float64], NDArray[np.int64]]:
    dt = dt_short * np.exp(-alpha * e) + dt_inf * (1 - np.exp(-alpha * e))
    mv = np.zeros(e.size, dtype=int)
...
def cov_W(j: int, t: NDArray[Any], gamma: float) -> NDArray[np.float64]:
    coeff = t[j] ** (1 - 2 * gamma) * (1 - 2 * gamma) / (1 - gamma)
    return np.asarray(
        coeff
        * (t[j] / t[j + 1 :]) ** gamma
        * special.hyp2f1(gamma, 1, 2 - gamma, t[j] / t[j + 1 :]),
        dtype=float,
    )


def cov_W_Z(
    j: int, t: NDArray[Any], H: float, rho: float, D_H: float
) -> NDArray[np.float64]:
    return np.asarray(
        rho * D_H * (t[j] ** (H + 0.5)) * np.ones(t.size - j), dtype=float
    )
...
def cov_Z_W(
    ...
    return np.asarray(
        rho * D_H * (t[j + 1 :] ** (H + 0.5) - (t[j + 1 :] - t[j]) ** (H + 0.5)),
        dtype=float,
    )


def cov_Z(j: int, t: NDArray[Any]) -> NDArray[np.float64]:
    return np.asarray(t[j] * np.ones(t[j + 1 :].size), dtype=float)


# Vanilla Price Calculation for matrices of Spot (and K an array)
# Call for K > S_0, put for K < S_0
def MC_price(S: NDArray[Any], K: NDArray[Any], S_0: float) -> NDArray[np.float64]:
    m = S.shape[0]
    l = K.size
    C: np.ndarray
    if S.ndim > 1:
...
def BSM_call_price(vol: float, S_0: float, K: float, T: float) -> float:
    d1 = np.divide(np.log(np.divide(S_0, K)) + 0.5 * vol * vol * T, vol * np.sqrt(T))
    d2 = d1 - vol * np.sqrt(T)
    p = S_0 * stats.norm.cdf(d1, loc=0, scale=1) - K * stats.norm.cdf(
        d2, loc=0, scale=1
    )
    return p
...
def BSM_put_price(vol: float, S_0: float, K: float, T: float) -> float:
    d1 = np.divide(np.log(np.divide(S_0, K)) + 0.5 * vol * vol * T, vol * np.sqrt(T))
    d2 = d1 - vol * np.sqrt(T)
    p = K * stats.norm.cdf(-d2, loc=0, scale=1) - S_0 * stats.norm.cdf(
        -d1, loc=0, scale=1
    )
    return p
...
def error_imp_vol(vol: float, S_0: float, K: float, T: float, price: float) -> float:
    if K >= S_0:
...
def BSM_imp_vol(S_0: float, K: float, T: float, price: float) -> float:
    try:
        f_low = error_imp_vol(0.0001, S_0, K, T, price)
        f_high = error_imp_vol(5.0, S_0, K, T, price)
        # Require a sign change in the bracket for a safe root find
        if f_low * f_high > 0:
            return float(np.nan)
        root = optimize.brentq(
            lambda v: error_imp_vol(v, S_0, K, T, price), 0.0001, 5.0
        )
        return float(root)
    except Exception as e:
        logging.debug(
            "BSM_imp_vol solver failed for K=%s T=%s price=%s: %s", K, T, price, e
        )
        return float(np.nan)
...
def arrays_control(
    expiries_nan: np.ndarray, forward_input_nan: np.ndarray, xi_input_nan: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    a = np.isfinite(expiries_nan)
    b = np.isfinite(forward_input_nan)
    c = np.isfinite(xi_input_nan)
    index_intersection = np.logical_and(a, np.logical_and(b, c))
...
def interpolating_xi(
    xi_input: np.ndarray, expiries: np.ndarray, mv: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    xi_values = np.diff(xi_input * expiries, prepend=0) / np.diff(expiries, prepend=0)
    xi = np.zeros(int(mv.sum()) + 1)
    xi[0 : mv[0] + 1] = xi_values[0]
    cumsum_mv = np.cumsum(mv)
    for i in range(1, mv.size):
        xi[cumsum_mv[i - 1] : cumsum_mv[i]] = xi_values[i]
    xi[-1] = xi[-2]
    return xi, xi_values


def xi_function(
    x: float, t: np.ndarray, expiries_index: np.ndarray, xi_values: np.ndarray
) -> float:
    ...
    for i in range(expiries_index.size - 1):
        if t[expiries_index[i] - 1] <= x < t[expiries_index[i] + 1]:
            mom_ang = (xi_values[i + 1] - xi_values[i]) / (
                t[expiries_index[i] + 1] - t[expiries_index[i] - 1]
            )
...
def std_integrand(
    ...
    eta: float,
    t: np.ndarray,
    expiries_index: np.ndarray,
    xi_values: np.ndarray,
) -> float:
    gamma = 0.5 - H
    coeff = xi_function(y, t, expiries_index, xi_values) * xi_function(
        x, t, expiries_index, xi_values
    )
    ...
    return float(
        ...
        * np.exp(
            ...
            * y ** (1 - 2 * gamma)
            * (y / x) ** gamma
            * special.hyp2f1(gamma, 1, 2 - gamma, y / x)
        )
    )
    ...
    for i in range(IV_skew.shape[0]):
        for j in range(IV_skew.shape[1]):
            if np.isnan(K_skew[i, j]):
                ...
                IV_skew[i, j] = BSM_imp_vol(
                    S_0, K_skew[i, j], expiries[i], C_skew[i, j]
                )
    return IV, IV_skew
...
def Sigma_Taylor_coefficients(H, rho, eta, xi, expiries):
    ...
    K1_Kbar1 = (
        special.beta(H + 1.5, H + 1.5) * (2 * H) / ((H + 0.5) * (H + 0.5))
    )  # <K1,K_bar 1>
    ...
    Sigma0_dd = sigma0_d * sigma0_d / (
        sigma0**3
    ) * Sigma0_dd_term + sigma0_dd * rho * rho * K12_1 / (sigma0 * sigma0)
    D_K_rho = K21_1 - K12_1 + rho * rho * (3 * K1_1 * K1_1 - K12_1 - 2 * K1_Kbar1)
    D_bar_K_rho = K21_1 - rho * rho * K12_1
    a_0 = (
        sigma0_d * sigma0_d * D_K_rho
        + sigma0 * sigma0_dd * D_bar_K_rho
        + sigma0 * sigma0_dot / (H + 0.5)
    )
    if H == 0.5:
...
def imp_vol_approx(K, K_skew, S_0, expiries, xi, H, rho, eta, tenor_len):
    ...
    IV_approx = (
        ...
        + Sigma0_d * x
        + 0.5 * Sigma0_dd * x * x
        + 0.5 * a_0 / Sigma0 * expiries[:, np.newaxis] ** (2 * H)
    )
...
def df_diagn_creation(
    dK_skew, dt_short, dt_inf, alpha, total_time, code_version, Sigma0_d, Sigma0_dd, a_0
):
    diagn_dic = {}
    ...
    diagn_dic["RunningDate"] = [
        datetime.today().strftime("%d-%m-%Y"),
        "",
    ]  #  strftime('%x'), see https://strftime.org/
...
def df_integ_var_std_creation(
    logS, logv, xi_input, xi_values, t, expiries, xi, H, eta, T
):
    expiries_index = np.where(np.isin(t, expiries))[0]
    ...
    int_subranges = np.hstack(
        [
            0,
            t[expiries_index_T[:-1] - 1],
            t[expiries_index_T[:-1] + 1],
            t[expiries_index_T[-1]],
        ]
    )
    ...
    for i in range(int_subranges.size - 1):
        tmp = (
            ...
            + integrate.dblquad(
                ...
                lambda x: 0.0,
                lambda x: x,
                args=(H, eta, t, expiries_index, xi_values),
            )[0]
        )
...
def df_term_creation(expiries_nan, forward_input_nan, xi_input_nan):
    ...
    df_term = pd.DataFrame(
        data=np.concatenate([tenor_df, forw_df, xi_df], axis=1),
        index=None,
        columns=col_names,
    ).T
    return df_term
...
def df_strikes_creation(K, strikes, S_0, tenor_len):
    ...
    if K_perc.shape[0] < tenor_len:
        ...
        K_df = K_perc
    col_names = ["FwdMness"] * K_perc.shape[0] + [np.nan] * (
        K_df.shape[0] - K_perc.shape[0]
    )
...
def df_IV_creation(IV, strikes, tenor_len, c=""):
    ...
    if c == "Approx":
        df_IV.index.name = "Approximation"
    return IV_df, df_IV


def df_skew_smile_creation(
    skew, smile, IV_skew, IV_df, strikes, dK_skew, expiries_nan, tenor_len
):
    ...
    col_names = col_names + [
        str((1 - dK_skew) * 100) + "%Vol",
        "ATM_vol",
        str((1 + dK_skew) * 100) + "%Vol",
    ]
    df_skew_smile = pd.DataFrame(
        data=np.concatenate(
            [
                atm_vol_df,
                skew_df_df,
